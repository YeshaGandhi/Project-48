var elephantImage, hippoImage, monkeyImage, pandaImage,hunter1,hunter2, hunterImage, panda, elephant,hippo,monkey, wallGroup,animalGroup,hunterGroup, edges
...
var monkey2,hippo2,gameOver,gameOverImage,sound,black,blackImage;
var score=0
...
sound=loadSound("/images/forest.wav")
...
function setup() {
  ...
  hippo2=createSprite(340,550);
  hippo2.addImage(hippoImage)
  hippo2.scale=0.13
  ...
  monkey2=createSprite(445,530);
  monkey2.addImage(monkeyImage)
  monkey2.scale=0.14
...
if(panda.isTouching(hunter1)||panda.isTouching(hunter2)){
...
black=createSprite(windowWidth/2,windowHeight/2);
...
if(panda.isTouching(monkey)){
  score=score+10
  monkey.destroy();
}

if(panda.isTouching(monkey2)){
  score=score+10
  monkey2.destroy();
}
if(panda.isTouching(hippo)){
  score=score+20
  hippo.destroy();
}
if(panda.isTouching(hippo2)){
  score=score+20
  hippo2.destroy();
}
if(panda.isTouching(elephant)){
  score=score+30
  elephant.destroy();
}
if(panda.isTouching(elephant2)){
  score=score+30
  elephant2.destroy();
}
